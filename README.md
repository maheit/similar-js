...
console.log(isSimilar([11, 3], [21, 1, 3, 11], { swapCompArray: false, exactArray: false })); // true, because all the items in first argument array present in second argument array.

console.log(isSimilar({ name: "Maheshwaran G" }, {} , { exactObject: false, swapCompObject: false })); // false, because second argument object atleast should include all the property of first argument object.

console.log(isSimilar({ name: "Maheshwaran G" }, {} , { exactObject: false })); // true, since swap of argument possible due to default swapComparisonObject as true so second argument has the minimum property than first then it means another object (first argument) has all the property of seconf argument object so it return true.
